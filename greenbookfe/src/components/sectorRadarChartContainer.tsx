...
const SectorRadarChartContainer = ({ type, order }: Props) => {
  ...
  //                 let averageA = topWordsA.reduce((x: any, y: any) => x + y) / topWordsA.length
  //                 let averageB = topWordsB.reduce((x: any, y: any) => x + y) / topWordsB.length
  ...
  useEffect(() => {
    let tempData: any[] = [];
    Object.keys(apiResp).forEach((entry) => {
      //@ts-ignore
  ...
  //             setDataToDisplay(data.slice((maxReportsToDisplay * (page - 1)), (maxReportsToDisplay * page)))
  //         })
  //     } else {
  //         if (order === 'best') {
